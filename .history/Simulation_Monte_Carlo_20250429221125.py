...
def simulate_returns_with_copula(
    ...
    try:
        if copula_type == CopulaType.GAUSSIAN and n_assets >= 2:
            ...
            copula.fit(returns.values)  # Convert DataFrame to numpy array
    ...
    # Ensure we have a numpy array before reshaping
...
def monte_carlo_sim(
    ...
    for i in range(params.n_simulations):
        for t in range(1, params.time_horizon + 1):
            portfolio_return = np.dot(weights, asset_returns[i, t-1])
