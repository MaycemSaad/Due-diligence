...
with st.expander("🧠 Explain AI Prediction (Feature Importance)"):
    ...
    try:
        # Step 1: Prepare data - ensure we have enough samples
        if len(X) < 100:
            st.warning("For better SHAP explanations, we recommend at least 100 data points")
            min_samples = min(30, len(X))
            X_flat = X.reshape((X.shape[0], X.shape[1]))[:min_samples]
        else:
            X_flat = X.reshape((X.shape[0], X.shape[1]))[:100]  # Use first 100 samples

        # Step 2: Create a simplified wrapper model
        def model_predict(x):
            return model.predict(x.reshape(-1, look_back, 1), verbose=0).flatten()

        # Step 3: Use PartitionExplainer which is more robust for neural networks
        explainer = shap.Explainer(
            model_predict, 
            X_flat,
            feature_names=[f"Day-{i}" for i in range(look_back, 0, -1)]
        )
        ...
        fig, ax = plt.subplots()
        shap.summary_plot(shap_values, X_flat, show=False, plot_type="bar")
        st.pyplot(fig)

        # Step 6: SHAP Force Plot for Single Sample
        st.subheader("⚡ SHAP Force Plot for a Random Prediction")
        random_idx = np.random.randint(0, len(X_flat))
        
        # Create force plot with the new API
        force_plot = shap.plots.force(
            shap_values[random_idx],
            matplotlib=False,
            figsize=(15, 3)
        )
        components.html(force_plot.html(), height=200, scrolling=True)
        
    except Exception as e:
        st.error(f"An error occurred while generating SHAP explanations: {str(e)}")
        st.warning("""
            Common issues:
            1. Too few samples - try with more historical data
            2. Model didn't train properly - try increasing epochs
            3. Memory limitations - try reducing the number of samples
            """)
