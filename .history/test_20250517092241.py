def show_due_diligence_app():
    ...
    def voice_chat_loop():
        """Main loop for voice conversation"""
        st.header("🎙️ Voice Assistant Mode")
        st.write("Ask your due diligence questions verbally")
        ...
        if st.button("🎤 Start Voice Chat", key="start_voice"):
            while True:
                ...
                if not speak_response(answer):
                    st.warning("Couldn't generate voice response, displaying text only")
                ...
                if not st.button("🎤 Ask another question", key="continue_voice"):
                    break
...
# 💬 Affichage messages + Lecture audio + Filtrage par tag (Q&A associés)
    ...
    while i < len(st.session_state.history):
        ...
        # Add voice assistant button to sidebar
    st.sidebar.title("🎙️ Voice Assistant")
    if st.sidebar.button("🗣️ Talk to AI Assistant"):
        voice_chat_loop()
