...
def show_signup_page():
    ...
    try:
        ...
        st.markdown(f"""
            <div class="login-header">
                <img src="{logo_base64}" width="320" style="margin-bottom: 2rem;" />
    ...
    if st.button("Create account "):
        if not first_name or not last_name or not email or not password or not confirm:
            ...
            st.success(f"✅ Account created successfully, {first_name} !")
    ...
    with col1:
        if st.button("🔐 Already Signed Up ? Log In "):
...
def show_login_page():
    ...
    st.markdown('<div class="auth-title">🔐 Secure connection </div>', unsafe_allow_html=True)
    ...
    if st.button("Log In"):
        ...
        if user and verify_password(password, user["password"]):
            ...
            if st.session_state.user["role"] == "admin":
                st.success("✅ Login successful. Redirectig to Dashboard Admin...")
                st.session_state.page = "admin_dashboard"  # ➡️ you must manage this page
                st.rerun()
            else:
                st.success("✅ Login successful. Redirecting to user application..")
...
def show_verification_page():
    ...
    st.markdown("""
        <div style='text-align: center; margin-bottom: 2rem; animation: fadeIn 1s ease-in-out;'>
            <h2 style='color:#2E3B4E;'>🪪 Identity Verification Required</h2>
            <p style='color:gray; font-size: 1rem;'>For your security and compliance, we need to confirm your identity before granting full access.</p>
        </div>
    """, unsafe_allow_html=True)

    # Info block with styled box
    st.markdown("""
        <div style='
        ...
        '>
            ...
            🔐 <strong>Why this step?</strong> To prevent fraud and ensure that every user is securely onboarded into our intelligent audit ecosystem.
            </p>
            <ul style='margin-left: 1rem; color:#333;'>
                <li>📷 Upload a clear photo of your national identity card (front side).</li>
                <li>🔒 Your document will be securely processed and never shared.</li>
                <li>⏱️ The check usually takes a few seconds.</li>
            </ul>
        </div>
    """, unsafe_allow_html=True)
    ...
    # Confirmation CTA
    st.markdown("""
        <div style='text-align: center; margin-top: 2rem; animation: fadeIn 1.5s ease-in-out;'>
            <p style='color: #2E3B4E; font-size: 1rem;'>Once you’ve reviewed this information, you may proceed to the platform.</p>
            <button style="
                background-color: #0066ff;
                color: white;
                font-size: 1rem;
    ...
    # For Streamlit action
    if st.button("✅ Proceed (Streamlit Trigger)"):
