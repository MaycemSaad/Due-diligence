...
from Simulations import CryptoDashboard  # Assuming your class is in crypto_dashboard.py
...
def main():
    ...
    if st.sidebar.button("📊 Generate Full Report (PPTX)"):
        with st.spinner("Generating comprehensive PowerPoint report..."):
            try:
                # Exemple d’appel
            report_path = generate_pptx_report_fixed(analyzer=self.analyzer, ticker="BTC-USD", crypto_mapping=CRYPTO_MAPPING)
