def show_due_diligence_app():
    ...
    # 📥 Charger toutes les sessions pour l'utilisateur
    all_sessions = list(collection.find(
        {"user_email": st.session_state.user},
        {"messages": 1}
    ))
    ...
    # 🧠 Ajouter les messages de la session actuelle (non encore sauvés)
    all_messages.extend(st.session_state.history)

    # 💬 Affichage filtré (Q&A en paires)
    i = 0
    while i < len(all_messages):
        user_msg = all_messages[i]
        assistant_msg = all_messages[i+1] if i+1 < len(all_messages) else None

        # Skip si pas pair valide
        if user_msg["role"] != "user" or not assistant_msg or assistant_msg["role"] != "assistant":
            i += 1
            continue

        # Skip si tag sélectionné ne correspond pas
        if selected_tag != "All" and selected_tag not in assistant_msg.get("tags", []):
            i += 2
            continue
