def show_due_diligence_app():
    ...
    try:
        pygame.mixer.init()
        st.success("✅ Audio mixer initialized successfully.")
    except Exception as e:
        st.error(f"❌ Failed to initialize audio mixer: {e}")

    # 🔊 Écoute du microphone
    def listen_to_user():
        """Capture l'audio de l'utilisateur et convertit en texte"""
        r = sr.Recognizer()
        
        try:
            with sr.Microphone() as source:
                # ✅ Ajustement pour le bruit ambiant pendant 2 secondes
        ...
        try:
            # 🔄 Conversion en texte
            text = r.recognize_google(audio, language="fr-FR")  # 🌍 Langue changée pour le français
            st.success(f"🎤 Vous avez dit: {text}")
            return text
        except sr.UnknownValueError:
            st.error("🔇 Impossible de comprendre l'audio. Parlez plus clairement.")
            return None
        except sr.RequestError as e:
            st.error(f"🔇 Problème de connexion avec le service de reconnaissance vocale : {e}")
            return None
        ...
        with st.spinner("🔊 Génération de la réponse vocale..."):
            try:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp:
                    tts = gTTS(text=text, lang='fr', slow=False)  # 🌍 Langue changée pour le français
                    tts.save(tmp.name)
    ...
    def voice_chat_loop():
        """Handle one iteration of voice conversation"""
        st.header("🎙️ Voice Assistant Mode")
        
        # Adjust microphone for ambient noise first
        with st.spinner("🔊 Adjusting microphone..."):
            try:
                r = sr.Recognizer()
                with sr.Microphone() as source:
        ...
        # Listen to user with timeout and phrase limit
        try:
            with sr.Microphone() as source:
            ...
            st.warning("⏱️ Listening timed out - try speaking louder")
            return
        except sr.UnknownValueError:
        ...
        # Voice response with retry logic
        if not speak_response(answer):
            st.warning("Voice response failed - trying one more time...")
            time.sleep(1)
            if not speak_response(answer):
                st.error("Couldn't generate voice response")
    ...
    def test_microphone():
        ...
        st.sidebar.title("🎙️ Voice Assistant - Test Microphone")

        # Bouton pour tester le microphone
        if st.sidebar.button("🔍 Tester le microphone", key="mic_test"):
            test_microphone()

        # Bouton pour écouter le fichier audio
        if st.sidebar.button("🔊 Écouter le fichier audio", key="listen_audio"):
            try:
                audio_file = open('test_audio.wav', 'rb')
                st.audio(audio_file.read(), format='audio/wav')
            except FileNotFoundError:
                st.error("❌ Le fichier audio n'a pas été trouvé.")

        # Bouton pour tester la reconnaissance vocale
        if st.sidebar.button("🗣️ Tester la reconnaissance vocale", key="recognize_voice"):
            boosted_audio_path = increase_volume("test_audio.wav")
            if boosted_audio_path:
                recognize_from_file(boosted_audio_path)
    ...
    if st.sidebar.button("🔍 Tester le microphone"):
        test_microphone()
