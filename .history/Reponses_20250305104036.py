...
def generate_answer_with_ollama(question, context):
    """🤖 Generates an AI-powered answer with a fallback mechanism."""
    prompt = f"""
    You are an expert in digital finance and due diligence. Answer the question based on the provided context.

    [CONTEXT]: {context if context else "No specific context is available."}

    [QUESTION]: {question}

    [INSTRUCTIONS]:
    - If the context is relevant, provide a direct answer.
    - If context is missing, make an educated guess based on common industry knowledge.
    - If the question cannot be answered at all, suggest additional information that would help.
    - Keep responses clear, factual, and structured.
    ...
    # Ensure an answer is given
    return answer if answer else "I couldn't find a direct answer based on the available context. Please provide more details or refine your question."
