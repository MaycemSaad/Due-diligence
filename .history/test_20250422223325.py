...
# 💬 Historique sessions
chat_sessions = list(
    collection.find({"user_email": st.session_state.user})
    .sort("timestamp", -1)
    .limit(10)
)
...
for session in chat_sessions:
    ...
    first_question = next(
        (m["content"] for m in session.get("messages", []) if m.get("role") == "user"), None
    )

    if first_question:
        session_names.append(first_question[:50] + "...")
    else:
