...
with st.expander("🧠 Explain AI Prediction (Feature Importance)"):
    ...
    # Step 6: SHAP Force Plot for a random prediction
    st.subheader("⚡ SHAP Force Plot for Random Prediction")

    # Pick one random sample
    random_idx = np.random.randint(0, len(test_samples))
    sample_to_explain = test_samples[random_idx].reshape(1, -1)

    # Explain single sample
    shap_value_single = explainer.shap_values(sample_to_explain, nsamples=100)

    # Create force plot in HTML
