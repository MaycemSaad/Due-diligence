...
if st.session_state.page == "login":
    ...
    with tabs[2]:
        ...
        <div class="card">
            <h3>Our Services</h3>
            <p>Explore our solutions tailored to enhance your investment strategies.</p>
        </div>
        """, unsafe_allow_html=True)

        st.subheader("🔹 Standard Services")
        cols1 = st.columns(3)
        standard_services = [
            ("🔍 Due Diligence Automation", "Accelerate your financial analysis with AI."),
            ("📈 Predictive Analytics", "Get forecasts based on market behaviors."),
            ("🛡️ Smart Compliance", "Ensure your assets comply with evolving regulations."),
        ]
        ...
        cols2 = st.columns(2)
        premium_services = [
            ("📊 Portfolio Stress Testing", "Simulate shocks to your investment portfolio."),
            ("🧠 Deep Behavioral Analytics", "AI-based prediction of asset volatility by investor behavior."),
        ]

        for col, (title, desc) in zip(cols2, premium_services):
            with col:
                st.markdown(f"""
                <div class="card">
                    <h3>{title}</h3>
        ...
        lottie_services = load_lottie_url("https://assets1.lottiefiles.com/private_files/lf30_jtkhrafn.json")
        if lottie_services:
            st_lottie(lottie_services, height=300, speed=1, quality="high")
        else:
