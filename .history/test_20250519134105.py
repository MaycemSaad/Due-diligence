def show_due_diligence_app():
    ...
    # 🎙️ Enhanced Voice Chat Loop with conversation flow
    def voice_chat_loop():
        ...
        # Continue options
        st.success("Voice conversation ready for next question")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🎤 Ask Another Question", key="voice_continue"):
                st.rerun()
        with col2:
            if st.button("❌ Exit Voice Mode", key="voice_exit"):
                st.session_state.voice_mode = False
                st.rerun()
    import speech_recognition as sr
    import streamlit as st
