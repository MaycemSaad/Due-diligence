    ...
    if not df.empty:
        ...
        if entities:
            ...
            for _ in range(10):  # Run animation for 10 iterations
                ...
                # Highlight most frequent word dynamically
                ax.text(
                    0.5, 0.02, f"Most Frequent Word: {most_frequent_word.upper()} ({most_frequent_count} times)", 
                    fontsize=16, fontweight='bold', color='red', ha='center', transform=ax.transAxes, 
                    bbox=dict(facecolor='black', alpha=0.3, edgecolor='red')
                )
