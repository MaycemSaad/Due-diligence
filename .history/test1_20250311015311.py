...
page = st.sidebar.radio("Select a Page", ["Overview", "Q&A Data", "Engagement Analysis", "Sentiment Analysis", "NER & Topics" , "Statistics", "Trending Topics", "Advanced Sentiment Analysis", "Data Export",  "User Interaction Trends", "Chatbot Simulation & A/B Testing", "AI Improvement Suggestions"])
...
# 📌 Sentiment Analysis with Timeline
    ...
    fig_sentiment = px.line(sentiment_trend, title="Sentiment Trend Over Time")
...
# 📌 Chatbot Simulation & A/B Testing
if page == "Chatbot Simulation & A/B Testing":
    st.header("🗣️ Chatbot Simulation & A/B Testing")
    user_input = st.text_input("Enter a question to test the chatbot:")
    if st.button("Test Chatbot"):
        chatbot_response_a = "Response A: " + user_input[::-1]  # Simulated response A
        chatbot_response_b = "Response B: " + user_input.upper()  # Simulated response B
        st.write("### Chatbot Responses")
        st.write("**A:**", chatbot_response_a)
        st.write("**B:**", chatbot_response_b)
        st.radio("Which response is better?", ["A", "B"], key="ab_test")

# 📌 AI Improvement Suggestions
elif page == "AI Improvement Suggestions":
    st.header("🚀 AI-Based Improvement Suggestions")
    
    # Detecting unanswered questions
    unanswered_df = df[df["answer"].isna() | (df["answer"] == "")]
    if not unanswered_df.empty:
        st.subheader("❌ Unanswered Questions")
        st.dataframe(unanswered_df[["question", "timestamp"]])
    else:
        st.success("No unanswered questions detected.")
    
    # Finding most negative feedback
    negative_feedback = df[df["sentiment_label"] == "Negative"]
    if not negative_feedback.empty:
        st.subheader("⚠️ Negative Feedback Detected")
        st.dataframe(negative_feedback[["question", "answer", "timestamp"]])
    else:
        st.success("No major negative feedback detected.")
    
    # Improvement Suggestions
    st.subheader("🔹 Suggested Improvements")
    st.write("- Train the chatbot on recent interactions to improve response accuracy.")
    st.write("- Enhance sentiment detection by incorporating a deep learning model.")
    st.write("- Implement a feedback system allowing users to rate responses.")        
