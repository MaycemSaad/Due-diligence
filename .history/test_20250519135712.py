def show_due_diligence_app():
    ...
    def init_audio():
        try:
            ...
            st.success(f"✅ Audio initialized using {mic_list[st.session_state.selected_mic_index]}")
            return True
    ...
    def listen_to_user():
        ...
        try:
            with selected_mic as source:
                ...
                calibration_status.success(f"🎤 Ready! Speak now... (Threshold: {current_threshold:.1f})")
                
                # Listen with visual feedback
                listening_status = st.empty()
                listening_status.info("🎤 Listening...")
    ...
    def voice_chat_loop():
        ...
        # Enhanced continue options
        st.success("✅ Voice conversation ready")
        cols = st.columns(3)
        with cols[0]:
