...
with st.expander("🧠 Explain AI Prediction (Feature Importance)"):
    ...
    try:
        ...
        if len(shap_values.shape) == 3:
            shap_values = shap_values[0]  # Take first output if 3D

        # Step 5: Plot SHAP Summary
        st.subheader("🔍 SHAP Feature Importance (Past Lags)")
        shap_plot = plt.figure()
        shap.summary_plot(shap_values, test_samples, show=False, plot_type="bar", 
                         feature_names=[f"Day-{i}" for i in range(X_flat.shape[1], 0, -1)])
        st.pyplot(shap_plot)
        ...
        explanation = shap.Explanation(
            values=shap_values[random_idx],
            base_values=explainer.expected_value,
            data=test_samples[random_idx],
            feature_names=[f"Day-{i}" for i in range(X_flat.shape[1], 0, -1)]
        )
        ...
        st.warning("This might happen with small datasets or certain model configurations.")
