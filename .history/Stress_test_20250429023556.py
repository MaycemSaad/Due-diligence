...
with st.expander("🧠 Explain AI Prediction (Feature Importance)"):
    ...
    # Step 6: SHAP Force Plot (corrected to work with single prediction)
    st.subheader("⚡ SHAP Force Plot for Random Prediction")

    import shap.plots

    random_idx = np.random.randint(0, len(test_samples))
    single_sample = test_samples[random_idx].reshape(1, -1)
    single_shap_value = explainer.shap_values(single_sample)

    # Generate force plot
    shap_html = shap.force_plot(
        explainer.expected_value, 
        single_shap_value[0],  # Correct: only one prediction
