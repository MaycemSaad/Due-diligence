def show_due_diligence_app():
    ...
    def render_tags(tags):
        tag_html = ' '.join([f'<span style="background-color:#343541;padding:5px 10px;border-radius:12px;color:white;font-size:12px;margin-right:5px;">{tag}</span>' for tag in tags])
    ...
    if st.sidebar.button("➕ New Chat"):
        # 1. Réinitialiser l'historique
        st.session_state.history = []

        # 2. Réinitialiser le contexte avec les textes nettoyés (optionnel)
        cleaned_texts_dir = "cleaned_texts"
        cleaned_texts = {
            txt_file: open(txt_file, "r", encoding="utf-8").read()
            for txt_file in glob.glob(f"{cleaned_texts_dir}/*.txt")
        ...
        # 3. Nouveau nom de session
        st.session_state.session_name = f"Session-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"

        # 4. Marquer comme reset pour afficher toast
        st.session_state["just_reset"] = True

        # 5. Supprimer toute sélection précédente (sinon `selectbox` recharge l'ancienne session)
        st.session_state["selected_session"] = None

        # 6. Rerun pour redémarrer proprement
        st.rerun()
    ...
    all_tags = sorted(set(tag for msg in st.session_state.history for tag in msg.get("tags", [])))
    ...
    # 💬 Historique sessions
    chat_sessions = list(
    collection.find({"user_email": st.session_state.user}, {"session": 1, "timestamp": 1})
    .sort("timestamp", -1)
    .limit(10)
)

    session_names = [s["session"] for s in chat_sessions]

    selected_session = st.sidebar.selectbox("📚 Load a past chat", session_names)
...
selected = st.sidebar.selectbox(
    "📚 Load a past chat", ["Choose an option"] + session_names,
    index=0 if not st.session_state.selected_session else session_names.index(st.session_state.selected_session) + 1,
    key="selected_session"
)

# ✅ Recharger la session si sélection changée
if selected != "Choose an option" and selected != st.session_state.session_name:
    doc = collection.find_one({"session": selected, "user_email": st.session_state.user})
    if doc:
    ...
    selected = st.sidebar.selectbox("📚 Load a past chat", session_names, key="selected_session")
    if selected and selected != st.session_state.session_name:
        doc = collection.find_one({"session": selected, "user_email": st.session_state.user})
        if doc:
            st.session_state.session_name = selected
            st.session_state.history = doc["messages"]
    ...
    if question:
        ...
        #log_event(st.session_state.user, "feedback_like", msg["content"])
        ...
        with st.spinner("⏳ Thinking..."):
            ...
            st.session_state.history[-1]["tags"] = tags  # Ajout dans l'historique
    ...
    # 💬 Affichage messages + Lecture audio + Filtrage par tag
    for i, msg in enumerate(st.session_state.history):

        # 🔍 Ne pas afficher si un tag est sélectionné et que ce message ne le contient pas
        if msg["role"] == "assistant" and selected_tag != "All":
            if selected_tag not in msg.get("tags", []):
                continue
        ...
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant":
                            # 👍 Bouton Like avec logging correct
                ...
                if st.button(f"📈 Evaluate & Improve {i}", key=f"judge_{i}"):
                    user_question = ""
                    for j in range(i-1, -1, -1):
                        if st.session_state.history[j]["role"] == "user":
                            user_question = st.session_state.history[j]["content"]
                            break

                    score, comment = evaluate_answer_with_ollama(user_question, msg["content"])
                    ...
                    if score < 7:
                        ...
                        improved_answer = improve_answer_with_ollama(user_question, msg["content"], context)

                        st.markdown("🔁 **Improved Answer:**")
                        st.markdown(improved_answer)

                        # Ajouter la nouvelle réponse avec tags
                        tags = classify_answer_tags(improved_answer)
