...
with st.expander("🧠 Explain AI Prediction (Feature Importance)"):
    ...
    # Prepare X_flat to 2D for SHAP
    X_flat = X.reshape((X.shape[0], X.shape[1]))

    # Use GradientExplainer because model is an LSTM
    explainer = shap.GradientExplainer(model, X)

    # Get SHAP values
    shap_values = explainer.shap_values(X)

    # ⚡ Correct the fact shap_values is a list
    if isinstance(shap_values, list):
        shap_values = shap_values[0]  # Take first output if list

    # Display SHAP summary plot
    st.subheader("🔍 SHAP Feature Importance (LSTM Prediction)")
    shap_plot = plt.figure()
    shap.summary_plot(shap_values, X_flat, show=False, plot_type="bar")
    st.pyplot(shap_plot)

    # Display Force Plot
    st.subheader("⚡ SHAP Force Plot for Random Forecast")
