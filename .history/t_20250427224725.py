...
if st.session_state.page == "login":
    ...
    with tabs[2]:
        ...
        for col, (title, desc) in zip(cols2, premium_services):
            with col:
                st.markdown(f"""
                <div style="background-color:#1a1a1a; border:1px solid #333; border-radius:12px; padding:30px; text-align:center; transition:0.3s; box-shadow: 0 6px 14px rgba(255,215,0,0.15);">
                    <h4 style="color: #F4C430;">{title}</h4>
                    <p style="color: #d0d0d0;">{desc}</p>
                </div>
                """, unsafe_allow_html=True)
        ...
        # Animation Services
        lottie_services = load_lottie_url("https://assets6.lottiefiles.com/packages/lf20_ntbzfunn.json")  # ✅ A working animation!
