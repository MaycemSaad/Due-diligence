def show_due_diligence_app():
    ...
    # 🔊 Enhanced Voice Listening
    def listen_to_user():
        """Capture user audio and convert to text with better error handling"""
        if not init_audio():
            return None
            
        r = sr.Recognizer()
        r.energy_threshold = 4000  # Better for office environments
        r.dynamic_energy_threshold = True
        r.pause_threshold = 0.8  # Longer pauses between phrases
        ...
        try:
            with sr.Microphone() as source:
                ...
                st.info("🎤 Speak now... (15 seconds max)")
            ...
            st.warning("⏱️ Listening timed out - please speak louder")
    ...
    # 🔊 Improved Speech Synthesis with file locking handling
    def speak_response(text):
        ...
        try:
            ...
            try:
                ...
                # Always stop and unload music
                pygame.mixer.music.stop()
                pygame.mixer.music.unload()
                
                # Try to delete the temp file (may fail on Windows)
                try:
                    os.unlink(temp_path)
                except:
    ...
    # 🎙️ Enhanced Voice Chat Loop
    def voice_chat_loop():
        """Handle voice conversation with better flow control"""
        ...
        if not speak_response(answer):
            st.warning("Voice response failed - displaying text only")
            
        # Continue options
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🎤 Ask Another", key="voice_continue"):
                st.rerun()
        with col2:
            if st.button("❌ Exit Voice", key="voice_exit"):
                st.session_state.voice_mode = False
                st.rerun()
