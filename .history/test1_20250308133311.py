...
if page == "Overview":
    ...
    st.write("This dashboard provides real-time insights into chatbot interactions, engagement levels, sentiment analysis, and named entity recognition trends. Use the filters to refine your data and explore detailed analytics.")

    col1, col2 = st.columns(2)
    
    with col1:
        st.metric(label="Total Interactions", value=len(df))
        st.metric(label="Average Response Length", value=round(df["response_length"].mean(), 2))
    ...
    fig_engagement = px.line(df, x="timestamp", y="response_length", 
