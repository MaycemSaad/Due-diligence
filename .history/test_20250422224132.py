def show_due_diligence_app():
    ...
    if st.sidebar.button("➕ New Chat"):
        ...
        st.rerun()
    # 💬 Ajout du titre de session personnalisé
    if "session_title" not in st.session_state:
        st.session_state.session_title = f"My Session {datetime.now().strftime('%Y-%m-%d %H:%M')}"

    st.session_state.session_title = st.sidebar.text_input("📝 Session Title", value=st.session_state.session_title, key="session_title_input")    
