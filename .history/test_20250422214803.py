def show_due_diligence_app():
    ...
    chat_sessions = list(
        collection.aggregate([
            {"$match": {"user_email": st.session_state.user}},
            {"$sort": {"timestamp": -1}},
            {"$limit": 10},
            {"$addFields": {
                "first_question": {
    ...
    # 🔠 Nom de session = première question (40 caractères max), sinon session ID
    session_names = [
        s["first_question"]["content"][:40] + "..." if "first_question" in s and s["first_question"] else s["session"]
        for s in chat_sessions
    ]
    session_ids = [s["session"] for s in chat_sessions]
