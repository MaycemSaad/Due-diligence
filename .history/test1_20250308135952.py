    ...
    if not df.empty:
        ...
        if entities:
            ...
            def update(frame):
                ...
                animated_wordcloud = WordCloud(width=800, height=400, background_color="white" if theme_mode == "Light" else "#0F172A", colormap="viridis").generate(" ".join(sampled_entities))
                ax.imshow(animated_wordcloud, interpolation="bilinear")
                ax.axis("off")

            fig, ax = plt.subplots(figsize=(10, 5))
            ani = animation.FuncAnimation(fig, update, frames=10, repeat=True, interval=1000)
            st.pyplot(fig)
