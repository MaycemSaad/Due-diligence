...
def generate_answer_with_ollama(question, context):
    ...
    - If the context is **missing or insufficient**, provide an **educated guess** based
...
st.markdown("""
    <style>
        ...
        .header { text-align: center; padding: 30px; background: linear-gradient(90deg, #003366, #0066CC); color: white; border-radius: 12px; }
        .card { background: white; padding: 20px; border-radius: 10px; box-shadow: 2px 4px 10px rgba(0, 0, 0, 0.1); margin-bottom: 20px; }
        .stButton>button { background-color: #0044CC !important; color: white !important; border-radius: 8px; padding: 12px; font-size: 16px; }
        .sidebar { background-color: #003366; color: white; padding: 20px; }
        .sidebar h1, .sidebar p, .sidebar label { color: white; }
        .sidebar .stButton>button { background-color: #0055FF !important; }
    </style>
""", unsafe_allow_html=True)

st.markdown("""
    <div class="header">
        <h1>💰 AI Due Diligence Chatbot</h1>
        <p>Your intelligent assistant for financial risk assessment</p>
    </div>
""", unsafe_allow_html=True)


# Upload & Display the Logo
...
st.sidebar.markdown(f"""
    ...
    <img src="{logo_path}" class="sidebar-logo">
    <p class="sidebar-title">📊 AI Financial Insights</p>
""", unsafe_allow_html=True)
try:
    logo = Image.open(logo_path)
    st.sidebar.image(logo, use_column_width=True)
except Exception as e:
    st.sidebar.warning("⚠️ Unable to load logo. Please check the file path.") 
# 🔹 Sidebar Content
st.sidebar.subheader("🔍 Recent Queries")
previous_qas = list(collection.find({}, {"question": 1}).sort("timestamp", -1).limit(5))
selected_past_question = st.sidebar.selectbox("", [qa['question'] for qa in previous_qas]) if previous_qas else None

st.sidebar.subheader("📊 Analysis Overview")
st.sidebar.write(f"**Total Questions Analyzed:** {collection.count_documents({})}")

if st.sidebar.button("📥 Export Data (CSV)"):
    df = pd.DataFrame(previous_qas)
    df.to_csv("qa_data.csv", index=False)
    st.sidebar.download_button("Download CSV", "qa_data.csv", file_name="qa_data.csv")
