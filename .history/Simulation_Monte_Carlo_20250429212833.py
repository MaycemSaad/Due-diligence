# Full advanced and corrected version of the institutional crypto risk platform in Streamlit
...
from riskfolio.Portfolio import Functions as RiskFunctions
import json
import base64
...
if copula_type == "Gaussian":
    ...
    st.warning(f"{copula_type} copula not supported yet. Falling back to Cholesky decomposition.")
...
def calculate_risk_metrics(paths):
    ...
    sortino = RiskFunctions.SortinoRatio(returns) if std_returns > 0 else 0
...
if st.sidebar.button("Run Advanced Simulation"):
    with st.spinner("Running institutional-grade simulation..."):
        ...
        import ace_tools as tools; tools.display_dataframe_to_user(name="Portfolio Paths", dataframe=pd.DataFrame(portfolio_paths))
        ...
        with tab2:
            ...
            if copula_type == "Gaussian":
                st.write("Using Gaussian Copula.")
            else:
                st.write("Using Cholesky decomposition (fallback).")
        ...
        with tab3:
            ...
            fig = go.Figure()
            fig.add_trace(go.Histogram(x=portfolio_paths[:, -1], nbinsx=50, name='Portfolio Value', marker_color='#1f77b4'))
            fig.update_layout(xaxis_title='Portfolio Value', yaxis_title='Frequency', bargap=0.1)
            st.plotly_chart(fig, use_container_width=True)
