    ...
    if not df.empty:
        ...
        if entities:
            ...
            for _ in range(10):  # Run animation for 10 iterations
                ...
                # Make the most frequent word bold
                plt.figure(figsize=(10, 5))
                plt.imshow(animated_wordcloud, interpolation="bilinear")
                plt.axis("off")
                plt.title(f"Most Frequent Word: **{most_frequent_word}** ({most_frequent_count} times)", fontsize=16, fontweight='bold', color='red')
