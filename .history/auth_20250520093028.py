...
def show_verification_page():
    ...
    st.info("Please upload a photo of your identity card to complete verification.")

    uploaded_file = st.file_uploader("Upload your ID card (JPG or PNG)", type=["jpg", "jpeg", "png"])

    if uploaded_file:
        st.image(uploaded_file, caption="ID card uploaded successfully.", use_column_width=True)
        if st.button("✅ Confirm and Continue"):
            st.session_state.page = "user_app"  # Or another page after verification
            st.success("Identity verified successfully. Redirecting...")
            st.rerun()
