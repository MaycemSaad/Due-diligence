...
st.markdown("""
    <style>
        ...
        .title { font-size: 48px; font-weight: bold; color: #FFD700; text-align: center; }
        .sub-title { font-size: 22px; text-align: center; color: #A9A9A9; }
        .response-box { background-color: #2C2F33; padding: 20px; border-radius: 12px; box-shadow: 4px 4px 15px rgba(255, 215, 0, 0.5); }
        .response-text { font-size: 20px; color: #FFFFFF; }
        .stButton>button { font-size: 18px !important; border-radius: 12px; padding: 12px 25px; background-color: #FFD700 !important; color: black !important; font-weight: bold; }
    </style>
...
with st.sidebar:
    st.title("📜 Past Questions & Analytics")
...
# 📝 User Input Field
question = st.text_input("💬 Type your question:", value=selected_past_question or "")
...
if txt_file:
    ...
    if st.button("📊 Summarize File"):
        ...
        st.markdown(f'<div class="response-box"><p class="response-text">{summary}</p></div>', unsafe_allow_html=True)
