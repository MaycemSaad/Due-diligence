...
if page == "Overview":
    ...
    if not df.empty:
        ...
        if entities:
            ...
            st.subheader("📝 Highlighted Named Entities")
            placeholder = st.empty()
            for entity in entities[:10]:  # Display top 10 entities one by one
                placeholder.markdown(f"<h3 style='text-align: center; color: #2ECC71;'>{entity}</h3>", unsafe_allow_html=True)
                time.sleep(0.5)
