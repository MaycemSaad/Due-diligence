    ...
    for rule in COMPLIANCE_RULES:
        ...
        if matches:
            for match in matches:
                ...
                total_risk += risk_weights.get(rule["risk"], 1)
...
def highlight_pdf(pdf_bytes, issues):
    """Highlight issues in the PDF"""
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    for page in doc:
        for issue in issues:
            if issue["text"]:
                instances = page.search_for(issue["text"])
...
def show_compliance_analyzer():
    ...
    st.subheader("Upload investment or crypto documents to detect regulatory compliance risks.")
    ...
    if uploaded_file:
        with st.spinner('🔎 Analyzing document...'):
            try:
                ...
                pdf_base64 = base64.b64encode(highlighted_pdf).decode()
                st.markdown(
                ...
                score_color = "#4CAF50" if score > 5 else "#F44336"
                st.markdown(
                    f"<h2 style='color:{score_color}; text-align:center;'>{analysis['score']}</h2>",
                    unsafe_allow_html=True
                )
                ...
                if analysis["issues"]:
                    for issue in analysis["issues"]:
                        st.warning(f"⚠️ '{issue['text']}' → {issue['regulation']} (Risk: {issue['risk']})")
                else:
