def show_due_diligence_app():
    ...
    from pptx.enum.shapes import MSO_SHAPE
    from datetime import datetime, timezone
    from ExtractionNLP import extract_text_from_pdf, clean_text, 
    from fpdf import FPDF
    from io import BytesIO
    ...
    def generate_compliance_pdf(session_name, summary, history, red_flags=None, reviewer="Due Diligence Bot"):
        ...
        if red_flags:
            pdf.set_font("Arial", 'B', 14)
            pdf.cell(0, 10, "⚠️ Red Flags Detected", ln=True)
            pdf.set_font("Arial", '', 12)
    ...
    def save_conversation(session_name, history):
        # S'assurer que chaque message a un champ "tags", même vide
        for msg in history:
            if "tags" not in msg:
                msg["tags"] = []
        ...
        collection.update_one(
            ...
            {"$set": {"messages": history, "timestamp": datetime.now(timezone.utc)}},
            upsert=True
        )
    ...
    if st.sidebar.button("➕ New Chat"):
        ...
        st.session_state.chat_context = dict(cleaned_texts)  # Reset chat context
        st.session_state.session_name = f"Session-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"  # New session name
        st.session_state["just_reset"] = True  # Mark that session has reset
        st.rerun()  # Rerun the app to reflect new session
    ...
    # 💬 Historique sessions
    chat_sessions = list(collection.find({}, {"session": 1, "timestamp": 1}).sort("timestamp", -1).limit(10))
    session_names = [s["session"] for s in chat_sessions]

    selected_session = st.sidebar.selectbox("📚 Load a past chat", session_names)
    if selected_session and selected_session != st.session_state.session_name:
        doc = collection.find_one({"session": selected_session})
        if doc:
    ...
    if st.sidebar.button("📄 Export PDF Report"):
            if st.session_state.chat_context and st.session_state.history:
                ...
                st.sidebar.download_button(
                    ...
                    data=pdf_file,
                    file_name=f"{st.session_state.session_name}_Compliance_Report.pdf",
                    mime="application/pdf"
                )
            else:
                st.sidebar.warning("❗ Upload a document and ask at least one question.")
