...
def visualize_data(data):
    """Affiche des graphiques professionnels sur les résumés."""
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.barplot(x=data["PDF Name"], y=data["Word Count"], palette="Blues_d", ax=ax)
    plt.xticks(rotation=45, ha="right")
    plt.title("Nombre de mots dans chaque document analysé")
    plt.xlabel("Fichiers PDF")
    plt.ylabel("Nombre de mots")
    st.pyplot(fig)

def process_pdfs():
    """Extrait, nettoie, résume et visualise chaque PDF."""
    pdf_data = []
...
st.set_page_config(page_title="AI Due Diligence Analysis", page_icon="📊", layout="wide")
...
with st.sidebar:
    ...
    if st.button("🔍 Start Analysis"):
        process_pdfs()
    st.write("📂 Upload a document for additional insights")
    txt_file = st.file_uploader("Upload a PDF", type=["pdf"])
    if txt_file:
        st.write("Processing uploaded document...")
        extracted_text = extract_text_from_pdf(txt_file)
        cleaned_text = clean_text(extracted_text)
        summary = summarize_text(cleaned_text)
