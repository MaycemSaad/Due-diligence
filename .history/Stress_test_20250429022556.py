...
with st.expander("🤖 Intelligent AI Future Prediction"):
    ...
    with st.spinner("Training intelligent AI model..."):
        ...
        look_back = 30  # last 30 days to predict next
        X, y = [], []
...
with st.expander("🧠 Explain AI Prediction (Feature Importance)"):
    st.info("Interpreting how past prices influenced the AI's decision...")

    import shap
    import matplotlib.pyplot as plt

    # Flatten X for explainability
    X_flat = X.reshape((X.shape[0], X.shape[1]))

    # Use KernelExplainer (more robust for LSTM)
    explainer = shap.KernelExplainer(model.predict, X_flat[:100])  # sample 100 points for efficiency

    # Explain
    shap_values = explainer.shap_values(X_flat[:100], nsamples=100)

    # ✅ Convert shap_values if it is a list
    if isinstance(shap_values, list):
        shap_values = shap_values[0]

    # Plot summary
    st.subheader("🔍 SHAP Feature Importance (Prediction Influences)")
    shap_plot = plt.figure()
    shap.summary_plot(shap_values, X_flat[:100], show=False, plot_type="bar", feature_names=[f"Day-{i}" for i in range(X_flat.shape[1])])
    st.pyplot(shap_plot)

    # Force Plot for 1 random sample
    st.subheader("⚡ SHAP Force Plot for 1 Random Forecast")
