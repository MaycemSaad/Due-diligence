def show_due_diligence_app():
    ...
    if not st.session_state.user:
        ...
        if st.button("Login"):
            ...
            if user and verify_password(password, user["password"]):
                # Store complete user document including email
    ...
    def save_conversation(session_name, history):
        ...
        if not st.session_state.get("user"):
            st.error("User not logged in during session save")
            return

        user_email = st.session_state.user["email"]  # Changed from st.session_state.user
        
        # Debug output
        print(f"\n🔵 Saving session for {user_email}")
        print(f"🔵 Session name: {session_name}")
        print(f"🔵 Messages to save: {len(history)}")

        try:
            result = collection.update_one(
                {
                    "session": session_name,
                    "user_email": user_email  # Changed to use email from user dict
                },
                {
                    "$set": {
                        "messages": history,
                        ...
                        "session_title": st.session_state.get("session_title", session_name)
                    }
                },
                upsert=True
            )
...
# 💬 Affichage messages + Lecture audio + Filtrage par tag (Q&A associés)
    ...
    if st.sidebar.button("👤 Generate Contrat"):
