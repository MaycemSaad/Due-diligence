...
today = pd.Timestamp.today().normalize().to_pydatetime()def fit_prophet(series, steps):
    ...
    forecast = forecast.loc[forecast.index >= today]  # ensure forecast starts from today
...
# Visualization
future_idx = pd.date_range(start=today, periods=forecast_days, freq='D')
fig = go.Figure()
...
fig.add_vline(x=today.isoformat(), line_width=2, line_dash="dash", line_color="gray", annotation_text="Today")
...
with st.expander("🤖 Intelligent AI Future Prediction"):
    ...
    with st.spinner("Training intelligent AI model..."):
        ...
        future_dates = pd.date_range(start=today, periods=prediction_days)
