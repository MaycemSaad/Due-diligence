...
candidate_labels = [
    "AML violation", "KYC non-compliance", "privacy violation", 
    "guaranteed investment return", "tax evasion", "fraudulent investment",
    "unregistered security", "non-compliance with GDPR", "offshore risk", "audit failure"
]

# --- Extract Text from PDF (full document) ---
def extract_text_from_pdf(pdf_bytes):
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    text = ""
    for page in doc:  # Read all pages
        text += page.get_text()
    return text[:10000]  # Limit if too long (for fast inference)

# --- Analyze text (Regex + AI) ---
def analyze_text(text, nlp_model):
    issues = []
    risk_weights = {"Critical": 3, "High": 2, "Medium": 1}
    total_risk = 0
    ...
    for rule in COMPLIANCE_RULES:
        ...
        for match in matches:
            risk = rule["risk"]
            total_risk += risk_weights.get(risk, 1)
            issues.append({
                "text": match.group(),
    ...
    ai_result = nlp_model(text, candidate_labels)
    for label, score in zip(ai_result['labels'], ai_result['scores']):
        if score > 0.7:  # Confidence threshold
            total_risk += 2  # Add semantic risk weight
            issues.append({
                "text": f"Semantic Risk: {label}",
                "regulation": "Detected via AI",
                "risk": "Medium",
                "recommendation": f"Review document for {label} issues",
                "source": "AI Semantic"
            })
...
# --- Streamlit App ---
def show_smart_compliance_analyzer():
    st.title("📜 Smart Compliance Analyzer PRO 🚀")
    st.subheader("Upload investment documents for advanced compliance risk analysis.")
    ...
    if uploaded_file:
        with st.spinner('Analyzing document...'):
            try:
                ...
                analysis = analyze_text(text, nlp_model)
                ...
                score_color = "#4CAF50" if int(analysis["score"].split("/")[0]) >= 8 else "#FF5722"
                st.markdown(f"<h2 style='color:{score_color}; text-align:center;'>{analysis['score']}</h2>", unsafe_allow_html=True)

                st.subheader("🔎 Identified Issues")
                if analysis["issues"]:
                    for issue in analysis["issues"]:
                        st.warning(f'{issue["source"]} ➔ "{issue["text"]}" ➔ {issue["regulation"]} (Risk: {issue["risk"]})')
                else:
