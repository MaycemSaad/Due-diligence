...
def show_generated_contrat():
    ...
    session_options = [f"{doc['session']} - {doc.get('session_title', 'No Title')}" for doc in sessions]
    selected_session_label = st.selectbox("🗂️ Select Session", session_options)

    # Get selected session document
    selected_doc = sessions[session_options.index(selected_session_label)]

    session_id = selected_doc["session"]
    messages = selected_doc.get("messages", [])
    
    last_answer = next((msg for msg in reversed(messages) if msg["role"] == "assistant"), None)
    summary_default = last_answer["content"] if last_answer else ""

    with st.form("contract_form"):
        name = st.text_input("👤 Full Name")
        email = st.text_input("📧 Email Address", user_email)
        summary = st.text_area("📌 Summary of Recommendation or Final Advice", value=summary_default, height=200)
