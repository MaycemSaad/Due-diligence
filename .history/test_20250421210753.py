def show_due_diligence_app():
    ...
    # 📎 Upload document
    st.sidebar.title("📎 Document Context")uploaded_files = st.sidebar.file_uploader("Upload .pdf or .txt files", type=["pdf", "txt"], accept_multiple_files=True)
    ...
    if uploaded_file:
        ...
        # 📥 Sauvegarder temporairement le fichier uploadé pour traitement par ExtractionNLP
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            tmp.write(uploaded_file.read())
            tmp_path = tmp.name

        # 📄 Extraction, nettoyage, résumé
        raw_text = extract_text_from_pdf(tmp_path)
        cleaned_text = clean_text(raw_text)
        summary = summarize_text(cleaned_text)

        # 💾 Enregistrer dans MongoDB (collection documents)
        ...
        # 🧠 Injecter dans le contexte du chatbot (pour usage immédiat dans les réponses)
        ...
        # 🔹 Optionnel : découpage du document
        chunks = chunk_text(doc_text)

        st.sidebar.subheader("💡 Suggested Questions")

        for idx, chunk in enumerate(chunks[:1]):  # on traite juste le premier chunk pour l'instant
            with st.spinner(f"Generating questions from chunk {idx+1}..."):
                result = force_question_output(chunk)

            if not result:
                st.sidebar.warning("❌ No valid questions generated.")
            else:
                for i, question in enumerate(result):
                    if st.sidebar.button(f"❓ {question}", key=f"suggested_q{i}"):
                        st.session_state["pending_question"] = question

        # 🔎 Analyse red flags
        detected_flags, flagged_sentences = detect_red_flags_with_highlight(doc_text)

        if detected_flags:
            st.sidebar.error(f"⚠️ Red flags detected: {', '.join(detected_flags)}")

            with st.sidebar.expander("🔍 Sentences with red flags", expanded=False):
                for sentence in flagged_sentences:
                    st.sidebar.markdown(f"• {sentence}", unsafe_allow_html=True)
        else:
            st.sidebar.success("✅ Document loaded without detected risks.")
