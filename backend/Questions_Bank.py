def Questions_bank():
    ...
    def display_questions(questions_bank, search_query="", selected_category="All"):
    ...
    if download and st.session_state.questions_bank:
        download_csv(st.session_state.questions_bank)

    # === Always show questions if exist ===
    if st.session_state.questions_bank:
        search_query = st.text_input("🔎 Search Questions...").lower()
        categories = ["All"] + list(st.session_state.questions_bank.keys())
