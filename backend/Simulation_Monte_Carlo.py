...
def monte_carlo_sim(
    returns: pd.DataFrame,  # Must be DataFrame for correlation
    weights: np.ndarray,
    ...
    # Validate inputs
    if not isinstance(returns, pd.DataFrame):
        raise TypeError("returns must be pandas DataFrame")
    if len(weights) != returns.shape[1]:
        raise ValueError("Weights length must match number of assets")
    
    # Simulate asset returns
    asset_returns = simulate_returns_with_copula(
        returns,  # Pass the DataFrame directly
        params.copula_type,
        params.n_simulations,
        params.time_horizon
    )
    
    # Vectorized portfolio path calculation
...
def calculate_risk_metrics(
    ...
    Args:
        paths: Array of portfolio paths (n_simulations x time_horizon + 1)
    ...
    returns = (final_values / paths[:, 0]) - 1  # Total period returns
    ...
    var = -sorted_returns[var_index] if var_index > 0 else 0
    cvar = -np.mean(sorted_returns[:var_index]) if var_index > 0 else 0
    
    # Drawdown calculation
    peak = paths.max(axis=1)
    trough = paths.min(axis=1)
    max_drawdown = np.max((peak - trough) / peak) if len(peak) > 0 else 0
    ...
    *Institutional-grade risk analytics for crypto assets*
    ...
    if st.sidebar.button("Run Advanced Simulation", type="primary"):
        with st.spinner("Running institutional-grade simulation..."):
            try:
                # Data simulation
                # 1. Load/generate your data as a DataFrame
                df = simulate_crypto_data(assets, include_jumps=params.include_jumps)

                # 2. Calculate returns (this will be a DataFrame)
                returns = df.pct_change().dropna()

                # 3. Run simulation (pass the DataFrame directly)
